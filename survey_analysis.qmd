...
survey_url <- "https://zenodo.org/records/19494321/files/survey_results_share.xlsx"
survey_file <- here("data", "survey_results_share.xlsx")
download.file(survey_url, destfile = survey_file, mode = "wb")

question_file <- here("question_data_share.xlsx")
...
```{r}
#| fig-width: 8
# For questions q_007 through q_023
barrier_questions <- paste0("q_", sprintf("%03d", 7:23))
...
# excluding barrier questions and country question (q_027)
mc_questions <- mc_questions[! mc_questions %in% c(barrier_questions, "q_027")]
...
compare_questions <- mc_questions[mc_questions != "q_006"]
...
for(qid in compare_questions) {
  ...
  question_list, 
  grouping_qid = "q_006",  # The grouping question
...
country_counts <- as.data.frame(table(answer_matrix[, "q_027"])) |>
  filter(Var1 != "") |>
...
```{r}
#| fig-width: 14
panel_1 <- plot_grid(mc_combined_plot_list[["q_006"]],
                     world_map_plot,
                     mc_combined_plot_list[["q_029"]],
                     mc_combined_plot_list[["q_028"]],
                     labels = LETTERS[1:4])

ggsave(here("output", "Fig1.eps"), device = "eps", width = 12, height = 8)
print(panel_1)

panel_2 <- plot_grid(barrier_questions_plot_combined,
                    mc_combined_plot_list[["q_024"]],
                    labels = LETTERS[1:2],
                    rel_widths = c(1.1, 0.9))

ggsave(here("output", "Fig2.eps"), width = 12, height = 8)
print(panel_2)

panel_3 <- plot_mc_combined(answer_df, "q_025", question_list,
                 grouping_qid = "q_006",
                 group1_value = "Analyst, Researcher, or Public healthcare professional",
                 group2_value = "Trainer or Educator",
                 frequency_cutoff = 4)

ggsave(here("output", "Fig3.eps"), width = 10, height = 8)
print(panel_3)

panel_4 <- plot_mc_combined(answer_df, "q_026", question_list,
                 grouping_qid = "q_006",
                 group1_value = "Analyst, Researcher, or Public healthcare professional",
                 group2_value = "Trainer or Educator",
                 frequency_cutoff = 2)

ggsave(here("output", "Fig4.eps"), width = 10, height = 5)
print(panel_4)

panel_5 <- plot_mc_combined(answer_df, "q_030", question_list,
                 grouping_qid = "q_006",
                 group1_value = "Analyst, Researcher, or Public healthcare professional",
                 group2_value = "Trainer or Educator",
                 frequency_cutoff = 2)

ggsave(here("output", "Fig5.eps"), width = 10, height = 9)
print(panel_5)
